  ...
  return (
    <>
      <div className='form-check'>
        <input
          className={`fs-6 form-check-input ${errors[name] && 'is-invalid'} `}
          type={type}
          name={name}
          id={id}
          value={value}
...
export const Input = ({ id, labelText, placeholder, register, type, errors, rules }) => {
  return (
    <>
      <label htmlFor={id} className='form-label' style={{fontSize:"14px"}}>
        {labelText}
      </label>
      <input
        id={id}
        ...
        className={`shadow-sm form-control ${errors[id] && 'is-invalid'}`}
        {...register(id, rules)}
        style={{ '--placeholder-color': 'rgba(0, 0, 0, 0.2)' }}
      />
